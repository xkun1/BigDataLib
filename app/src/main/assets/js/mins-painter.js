var MINPAINTERURL = "http://q1.chinabigdata.com/quote/SerialsRequest?code={0}&indicate-name=TrendLine&addition-indicate-names=Name,Now,High,Low,Open,PrevClose,Volume,Amount";
...
function getData() {
    ...
    var url = MINPAINTERURL.format(CurrentParsedUrl.param("c").toUpperCase());
    //Globals.Loading(true);
    DataService.GetData(url, true, "GET", "", function (data) {
        var chartdata = { quote: {}, mins: [] };
        chartdata.quote = { time: new Date(), open: data.Open, preClose: data.PrevClose, highest: data.High, lowest: data.Low, price: data.Now, volume: data.Volume, amount: data.Amount };
        $.each(data.serials.data, function (i, obj) {
            if (obj.time >= 93000000) {
                var entity = { price: obj.now, volume: obj.volume - (data.serials.data[i - 1]? data.serials.data[i - 1].volume : 0), amount: obj.amount - (data.serials.data[i - 1]? data.serials.data[i - 1].amount : 0) };
                chartdata.mins.push(entity);
            }
        });
        var c = document.getElementById("canvas");
        var ctx = c.getContext("2d");
        ctx.fillStyle = "white";
        ctx.fillRect(0, 0, 414, 310);
        ctx.clearRect(0, 0, 414, 310);
        //alert(chartdata.quote.open);
        $("#open").text(chartdata.quote.open.toFixed(2));
        $("#high").text(chartdata.quote.highest.toFixed(2));
        $("#low").text(chartdata.quote.lowest.toFixed(2));
        $("#volume").text(chartdata.quote.volume.toFixed(0));
        $("#name").text(data.Name);

        chart.paint(chartdata);
    }, "jsonp");
};
...
var chart = new minsChart('canvas', {
    ...
    topText: { font: '14px 宋体', color: 'black', region: { x: 28.5, y: 1.5, width: 350, height: 14 }, textBaseline: 'top' },
    minsChart: {
        region: { x: 56.5, y: 21.5, width: 210, height: 120 },
        ...
        yScalerLeft: { font: '8px Arial', region: { x: .5, y: 20, width: 50.5, height: 120 }, align: 'right', fontHeight: 6, textBaseline: 'top' },
        yScalerRight: { font: '8px Arial', region: { x: 264.5, y: 20, width: 40.5, height: 120 }, align: 'right', fontHeight: 6, textBaseline: 'top' }
    },
    xScaler: {
        font: '8px Arial', color: 'black',
        region: { x: 56.5, y: 145, width: 210, height: 20 },
        data: ['09:30', '10:30', '11:30/13:00', '14:00', '15:00']
    },

    volume: {
        region: { x: 56.5, y: 165.5, width: 210, height: 30 },
        bar: { color: 'green', width: 1 },
        borderColor: 'lightgray', splitLineColor: 'lightgray',
        yScaler: { font: '10px Arial', region: { x: .5, y: 162.5, width: 50.5, height: 30 }, color: 'black', align: 'right', fontHeight: 10, textBaseline: 'top' }
    }
});