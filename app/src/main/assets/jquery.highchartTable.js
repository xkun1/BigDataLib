...
(function($) {
		...
		this
				.each(function() {
					...
					if ($table.data('graph-container-before') != 1) {
						// Retrieve where the graph must be displayed from the
						// graph-container attribute
						var graphContainerSelector = $table
								.data('graph-container');
						if (!graphContainerSelector) {
							throw "graph-container data attribute is mandatory";
						}

						if (graphContainerSelector[0] === '#'
								|| graphContainerSelector.indexOf('..') === -1) {
							...
							while (currentGraphContainerSelector.indexOf('..') !== -1) {
								currentGraphContainerSelector = currentGraphContainerSelector
										.replace(/^.. /, '');
								referenceNode = referenceNode.parent();
							}

							graphContainer = $(currentGraphContainerSelector,
									referenceNode);
						}
					...
					ths
							.each(function(indexTh, th) {
								var $th = $(th);
								var columnScale = $th.data('graph-value-scale');

								var serieGraphType = $th.data('graph-type');
								if ($
										.inArray(serieGraphType,
												allowedGraphTypes) == -1) {
									serieGraphType = globalGraphType;
								}

								var serieStackGroup = $th
										.data('graph-stack-group');
								if (serieStackGroup) {
									graphIsStacked = true;
								}

								var serieDataLabelsEnabled = $th
										.data('graph-datalabels-enabled');
								...
								var thGraphConfig = {
									...
									yAxis : typeof yaxis != 'undefined' ? yaxis
											: 0,
									dashStyle : $th.data('graph-dash-style')
											|| 'solid',
								...
								if (typeof vlinex == 'undefined') {
									thGraphConfig.scale = typeof columnScale != 'undefined' ? parseFloat(columnScale)
											: 1;
									thGraphConfig.graphType = serieGraphType == 'column'
											&& isGraphInverted ? 'bar'
											: serieGraphType;
									...
									thGraphConfig.height = $th
											.data('graph-vline-height');
									thGraphConfig.name = $th
											...
											var serieConfig = {
												...
												dataLabels : {
													...
													align : $table
															...
															|| (globalGraphType == 'column'
																	&& isGraphInverted == 1 ? undefined
																	: 'center')
												}
											};
										...
										.each(function(indexTd, td) {
											...
											if (indexTd == 0) {
												...
												if (rawCellValue.length == 0) {
													...
													var cleanedCellValue = rawCellValue
															.replace(/ /g, '')
															.replace(/,/, '.');
													cellValue = Math
															.round(parseFloat(cleanedCellValue)
																	* column.scale
																	* 100) / 100;
													...
													var tdGraphName = $td
															.data('graph-name');
													var serieDataItem = {
														name : typeof tdGraphName != 'undefined' ? tdGraphName
																: rawCellValue,
														y : cellValue,
														x : dataGraphX
													// undefined if no x defined
													// in table
													};
					...
					for (yAxisNum = 1; yAxisNum <= nbYaxis; yAxisNum++) {
						var yAxisConfigCurrentAxis = {
							title : {
								text : typeof $table.data('graph-yaxis-'
										+ yAxisNum + '-title-text') != 'undefined' ? $table
										.data('graph-yaxis-' + yAxisNum
												+ '-title-text')
										: null
							},
							max : typeof $table.data('graph-yaxis-' + yAxisNum
									+ '-max') != 'undefined' ? $table
									.data('graph-yaxis-' + yAxisNum + '-max')
									: null,
							min : typeof $table.data('graph-yaxis-' + yAxisNum
									+ '-min') != 'undefined' ? $table
									.data('graph-yaxis-' + yAxisNum + '-min')
									: null,
							reversed : $table.data('graph-yaxis-' + yAxisNum
									+ '-reversed') == '1',
							opposite : $table.data('graph-yaxis-' + yAxisNum
									+ '-opposite') == '1',
							tickInterval : $table.data('graph-yaxis-'
									+ yAxisNum + '-tick-interval')
									|| null,
							labels : {
								rotation : $table.data('graph-yaxis-'
										+ yAxisNum + '-rotation') || 0
							},
							startOnTick : $table.data('graph-yaxis-' + yAxisNum
									+ '-start-on-tick') !== "0",
							endOnTick : $table.data('graph-yaxis-' + yAxisNum
									+ '-end-on-tick') !== "0",
							stackLabels : {
								enabled : $table.data('graph-yaxis-' + yAxisNum
										+ '-stacklabels-enabled') == '1'
							},
							gridLineInterpolation : $table.data('graph-yaxis-'
									+ yAxisNum + '-grid-line-interpolation')
									|| null
						};

						var callableYAxisFormatter = getCallable(table,
								'graph-yaxis-' + yAxisNum
										+ '-formatter-callback');
					...
					var defaultColors = [ '#4572A7', '#AA4643', '#89A54E',
							'#80699B', '#3D96AE', '#DB843D', '#92A8CD',
							'#A47D7C', '#B5CA92' ];
					var colors = [];

					var themeColors = typeof Highcharts.theme != 'undefined'
					...
					for ( var i = 0; i < 9; i++) {
						...
						colors
								.push(typeof $table.data(dataname) != 'undefined' ? $table
										.data(dataname)
										: typeof themeColors[i] != 'undefined' ? themeColors[i]
												: defaultColors[i]);
					}
					...
					var xAxisLabelsEnabled = $table
							.data('graph-xaxis-labels-enabled');

					var xAxisLabelStyle = {};
					var xAxisLabelFontSize = $table
					...
					var highChartConfig = {
						...
						chart : {
							...
							marginTop : typeof marginTop != 'undefined' ? marginTop
									: null,
							marginRight : typeof marginRight != 'undefined' ? marginRight
									: null,
						...
						legend : {
							...
							layout : $table.data('graph-legend-layout')
									|| 'horizontal',
						...
						xAxis : {
							categories : ($table.data('graph-xaxis-type') != 'datetime') ? xValues
									: undefined,
							type : ($table.data('graph-xaxis-type') == 'datetime') ? 'datetime'
									: undefined,
							reversed : $table.data('graph-xaxis-reversed') == '1',
							opposite : $table.data('graph-xaxis-opposite') == '1',
							showLastLabel : typeof $table
									.data('graph-xaxis-show-last-label') != 'undefined' ? $table
									.data('graph-xaxis-show-last-label')
									: true,
							tickInterval : $table
									.data('graph-xaxis-tick-interval')
									|| null,
							dateTimeLabelFormats : { // by default, we
														// display the day and
														// month on the datetime
														// graphs
								second : '%e. %b',
								minute : '%e. %b',
								hour : '%e. %b',
							...
							labels : {
								...
								align : $table.data('graph-xaxis-align')
										|| 'center',
								enabled : typeof xAxisLabelsEnabled != 'undefined' ? xAxisLabelsEnabled
										: true,
							...
							alternateGridColor : $table
									.data('graph-xaxis-alternateGridColor')
									|| null,
							title : {
								text : $table.data('graph-xaxis-title-text')
										|| null
							},
							gridLineWidth : $table
									.data('graph-xaxis-gridLine-width') || 0,
							gridLineDashStyle : $table
									.data('graph-xaxis-gridLine-style')
									|| 'ShortDot',
							tickmarkPlacement : $table
						...
						tooltip : {
							formatter : function() {
								if ($table.data('graph-xaxis-type') == 'datetime') {
									...
									var xValue = typeof xValues[this.point.x] != 'undefined' ? xValues[this.point.x]
											: this.point.x;
						...
						plotOptions : {
							...
							area : {
								lineWidth : lineWidth,
								shadow : typeof lineShadow != 'undefined' ? lineShadow
										: true,
								fillOpacity : $table
										.data('graph-area-fillOpacity') || 0.75
							},
							pie : {
					...
					$table.trigger('highchartTable.beforeRender',
							highChartConfig);
	...
	var parseDate = function(datetime) {
		...
		return new Date(parseInt(dateDayInfos[0], 10), parseInt(
				dateDayInfos[1], 10) - 1, parseInt(dateDayInfos[2], 10), min,
				hour);
	};
